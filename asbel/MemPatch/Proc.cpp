...
void interceptBreakpoints(DWORD pID, std::vector<patch_action>* patch_tasks) {
    // Attacher le processus pour le débogage
    ...
    while (debugging) {
        if (WaitForDebugEvent(&dbgEvent, INFINITE)) {
            ...
            if (dbgEvent.dwDebugEventCode == EXCEPTION_DEBUG_EVENT) {
                ...
                // Pour un hardware breakpoint d'exécution, l'exception est souvent EXCEPTION_SINGLE_STEP
            ...
            // Continuer l'exécution de l'événement traité
